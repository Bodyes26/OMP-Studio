...
- Safety confirmation modal on project and application exit with running and queued task protection: intercepts closing individual projects and the entire window (via onCloseRequested and the close button), warning the user if queued tasks or active runs are present; allows choosing whether to keep or discard queues and, if closed during active processing, aborts the process while automatically reinserting the interrupted task's prompt and settings at the top of `.omp/tasks.json` so it remains intact upon reopening.
