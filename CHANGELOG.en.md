...
- Five configurable Companion window layouts (Balanced, Dashboard, Attention Inbox, Compact, Launcher) selectable in "Settings → Companion", each with a visual preview card.
- Per-project task queue in the Companion: first three queued tasks and a "Run next" button when the agent is idle, with launch delegated to the main window.
- Status summary strip (working projects, attention requests, queued tasks) in Inbox and Launcher layouts.
...
- A disconnected or expired account no longer keeps its provider marked as active: model suggestions for a role also offered providers whose credentials had been switched off ("Disconnect" in the Providers tab, or a token flagged invalid by `omp`), and picking one of those models made the role fail on its first turn. Only providers with at least one valid credential now count, both in suggestions and in each provider's account count and "configured" state.
- Companion task field pills no longer offset the caret or overlap each other: the colored box used real horizontal space (padding, border, and bold text), so the caret sat a few pixels left of the characters and two consecutive directives such as `/plan /grill-me` ran into each other. The pill is now only color, outline, and vertical breathing room, so the painted text lines up exactly with what you type. A project whose name contains spaces (for example "Studio OMP") is also wrapped in the pill as a whole and removed entirely from the saved prompt: previously the pill stopped at the first word and the rest of the name stayed in the task text.
