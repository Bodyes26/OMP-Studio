...
### Added
- Project tab customization persistence: changes made to a project tab (custom name, badge/label sigla, hue and color mode, layout preferences, and task defaults) are permanently saved and automatically restored whenever the project is closed and reopened later. In the project picker (`+`), folders with custom names or labels display their badges and custom names, and can be searched directly using their assigned labels or names.
...
- `@file` mentions also work in the task editor and in the Companion, with the same palette as the Composer. In the Companion files are searched in the destination project, so `#project` comes first. Paths containing spaces are inserted in quotes (`@"Cruscotto PSR/Default.aspx"`) so they stay unambiguous.
- In chat bubbles and queued task previews every `@file` becomes a tag right where it was written: it shows the file name, the full path appears on hover, and a click opens it in the editor. Tags come back when a session is resumed.
...
- Isolated work lanes on the same repository. A row appears under the project bar only when the project has at least one lane besides Main: a title, a text status (working, needs input, review ready, conflict, integrating, idle) and “+ New lane”. The lane is a sibling worktree on branch `omp/lane-*`; `bin`, `obj`, `.vs`, `packages` and `node_modules` are neither copied nor junction-linked. A .NET project using PackageReference reuses the NuGet cache, while packages.config is reported with a disk estimate; local files the project needs to start (`Parametri.ini`, `.env`) are copied only after a confirmation that later lanes reuse. Clicking a queued task runs it on Main when Main is free; when Main is busy, Studio asks whether to open an isolated lane, and Shift+click opens it without asking. Auto-dispatch keeps at most one lane it opened itself, and from the third concurrent agent a warning lists the models and processes already running. Switching lanes realigns files, Git, the editor and the agent together; a question on a lane you are not looking at never steals focus and never enters the active chat: that tab breathes amber, and Companion quick reply stays available only when the lane is unambiguous. “Review & Integrate” shows the diff against the destination branch, with added and removed lines and the commands that actually ran. Integration starts only from your action and a conflict stays in the lane worktree (see below). From Companion, recovering from or dismissing a quota block acts on the blocked lane, even when it is not the one in front. Projects you already have open stay on Main and do not show the row until you create a lane.
- One-click lane integration, also from the chat. "Integrate" in the review dialog does everything: commits the work left in the lane, computes the merge without touching any checkout, lands a single squash commit on the target branch, then closes the lane session, removes the worktree and deletes the branch. You can ask an agent for the same thing ("integrate the lane"): the new `studio_lane_integrate` tool runs Studio's procedure instead of manual git commands. A chat card shows the commit and files integrated, with "Undo integration" that moves the branch back and keeps the work in `omp/restored-<lane>`.
- If the main branch has unsaved changes on files other than the lane's, integration goes ahead and leaves them intact; if they touch the same files, or the main agent is working, integration is queued and resumes on its own. On conflicts the files go back to the lane agent to resolve; after a resolution, integration asks for a confirmation click.
- Prototype Lab refounded as a specialized lane (`kind: 'lab'`): the Lab becomes a dedicated tab in the lane strip (`LaneStrip`), alongside Main and worktrees, without unmounting the workspace or killing running TUI terminals. Each prototype lives in an isolated workspace outside the repository with its own internal Git repository that records each user request as a recoverable revision. Sandboxed live preview in an opaque-origin iframe served by an on-demand local loopback server, supporting React 19 components, Tailwind CSS v4 styling, and third-party npm packages resolved automatically via esm.sh CDN. The prototype agent operates with standard tools confined to the workspace and read-only access to the host project, with automatic preview status checking and screenshots. Created prototypes are tracked in `.omp/lab/prototypes.json` (automatically ignored in `.gitignore`), reopenable from the project popover or used as reference by the main agent. Free-idea prototypes can be created from scratchpad (`Ctrl+Alt+P`) without a project folder, persisted locally and associable to open projects later.
- "New worktree lane" action directly in the project popover: allows creating the first manual worktree lane even when the lane strip is not yet visible.
...
- In the Companion the destination project is now written as `#project` instead of `@project`: `@` is reserved for files across the whole app.
...
- In a project menu, the Lab prototypes entry now opens its list in a side submenu next to the menu, which stays visible: the list used to replace the menu, shrinking it out from under the pointer so it closed immediately. The submenu also opens from the keyboard (Right arrow or Enter) and closes with Esc or Left arrow.
- "New Lab prototype" and "New worktree lane" from a project menu now work even if the menu closes while the prototype or lane is being created (moving the mouse off the menu was enough): the prototype used to be created but its lane never opened, leaving an orphan prototype in the list.
...
- Updating on the Nightly channel no longer fails with a 404 error right after a publication: Studio reads the nightly file list and manifest directly from the release instead of GitHub's cached copy, which for tens of minutes pointed at installers already removed, and no longer falls back to the installer of an older build.
- Starting a task in a new lane with Shift + click no longer opens the "Concurrent agents" dialog with a meaningless "agent number 1" count: the lane starts right away and the confirmation only appears from the third concurrent agent.
- The "Local files for the isolated lane" dialog no longer appears with "0 untracked files are needed": it only opens when there are new local files to authorize.
