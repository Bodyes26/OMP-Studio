...
- Safe native Git Worktree lane management: Studio resolves the real repository root even when a subfolder is open, creates sibling worktrees on `omp/lane-*` branches, preserves the operational subpath, and blocks removal of unregistered paths, dirty worktrees, or in-use resources without resorting to `--force`.
- Crash-safe lane persistence in `lanes.json`: order, status, workspace, session, and local profile survive restarts; Studio archives missing worktrees with an explicit reason and recovers recognizable unregistered worktrees without automatically deleting worktrees or branches.
- Multi-lane SessionRegistry and dual-surface handoff (Gate R27 W04): each project lane owns an independent `AgentSession` under the structured key `lane:<projectKey>:<laneId>`, binding the process to its dedicated workspace (worktree or main root), isolating transcripts, concurrent streaming, tokens, pending inputs, and abort/force-kill; TUI and GUI share the single logical process through seamless `--resume <sessionId>` handoff.
...
- Separated stable project identity from the lane workspace: existing projects now open automatically in the `main` lane with no visible change to the single-agent workflow.
