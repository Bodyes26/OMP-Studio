...
### Added
- Cinque layout configurabili per la finestra Companion (Bilanciata, Cruscotto, Inbox attenzione, Compatta, Launcher) selezionabili in «Impostazioni → Companion», con anteprima visiva di ciascun preset.
- Coda task per progetto nella Companion: prime tre attività in attesa e pulsante «Avvia prossimo» quando l'agente è libero, con avvio delegato alla finestra principale.
- Striscia di riepilogo stato (progetti al lavoro, richieste di attenzione, task in coda) nei layout Inbox e Launcher.
...
- «Impostazioni → Modelli → Provider & Custom» mostra di nuovo gli account collegati: la scheda dichiarava «Nessun account collegato a questo provider» e contava zero account per tutti i provider, perché l'apertura delle impostazioni non leggeva mai l'elenco delle credenziali (lo faceva solo il Companion). Ora la scheda rilegge gli account a ogni apertura, quindi compaiono subito anche quelli aggiunti da terminale dopo l'avvio di Studio.
- Un account disconnesso o scaduto non fa più risultare attivo il suo provider: i suggerimenti di modello per un ruolo proponevano anche provider le cui credenziali erano state spente («Disconnetti» nella scheda Provider, oppure token marcato non valido da `omp`), e assegnare uno di quei modelli faceva fallire il ruolo al primo turno. Ora conta solo chi ha almeno una credenziale valida, sia nei suggerimenti sia nel conteggio account e nello stato «configurato» di ogni provider.
- Le pastiglie del campo task nel Companion non sfalsano più il cursore e non si sovrappongono tra loro: il riquadro colorato occupava spazio orizzontale reale (imbottitura, bordo e testo in grassetto), così il cursore finiva qualche pixel a sinistra dei caratteri e due direttive consecutive come `/plan /grill-me` si accavallavano. Ora la pastiglia è solo colore, contorno e respiro verticale, quindi il testo dipinto combacia esattamente con quello digitato. Inoltre un progetto il cui nome contiene spazi (per esempio «Studio OMP») viene racchiuso per intero nella pastiglia e sparisce del tutto dal prompt salvato: prima la pastiglia si fermava alla prima parola e il resto del nome restava nel testo del task.
