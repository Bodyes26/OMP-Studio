...
- Gestione nativa e sicura delle corsie Git Worktree: Studio risolve la vera radice del repository anche quando è aperta una sottocartella, crea worktree fratelli su branch `omp/lane-*`, conserva il sottopercorso operativo e blocca rimozioni di percorsi non registrati, worktree sporchi o risorse in uso senza ricorrere a `--force`.
- Persistenza crash-safe delle corsie in `lanes.json`: ordine, stato, workspace, sessione e profilo locale sopravvivono al riavvio; Studio archivia con causa i worktree mancanti e recupera esplicitamente quelli riconoscibili ma non registrati, senza eliminare automaticamente worktree o branch.
- SessionRegistry multi-corsia e handoff a doppia superficie (Gate R27 W04): ogni corsia del progetto possiede un'istanza indipendente di `AgentSession` con chiave strutturata `lane:<projectKey>:<laneId>`, vincolando il processo al rispettivo workspace (worktree o radice principale), isolando transcript, streaming concorrente, token, input pendenti e abort/force-kill; TUI e GUI condividono lo stesso processo logico tramite handoff trasparente `--resume <sessionId>`.
...
- Separata l'identità stabile del progetto dal workspace operativo della corsia: i progetti esistenti si aprono automaticamente nella corsia `main` senza cambiamenti visibili al flusso a singolo agente.
