...
- Avvio più rapido: all'apertura di Studio parte subito solo l'agente del progetto attivo; gli agenti degli altri progetti si avviano uno alla volta in background, mentre un progetto con task in coda o scelto a mano parte immediatamente. Il controllo aggiornamenti di omp e il calcolo dei consumi partono dopo che la prima chat è pronta, e `omp --version` viene eseguito una sola volta.
- I file si aprono nell'editor senza aspettare git: il contenuto compare subito e gli indicatori delle modifiche a margine arrivano subito dopo. I file oltre 5 MB o binari mostrano un avviso invece di bloccare l'editor e non possono essere sovrascritti per errore.
...
- Nel menu di un progetto, la voce dei prototipi Lab apre l'elenco in un sottomenu laterale accanto al menu, che resta visibile: prima l'elenco sostituiva il menu, lo accorciava lasciando il puntatore fuori, e il menu si chiudeva subito. Il sottomenu si apre anche da tastiera (freccia destra o Invio) e si chiude con Esc o freccia sinistra.
- «Nuovo prototipo Lab» e «Nuova corsia worktree» dal menu di un progetto funzionano anche se il menu si chiude mentre il prototipo o la corsia vengono creati (basta spostare il mouse fuori dal menu): prima il prototipo veniva creato ma la corsia non si apriva, e restava un prototipo orfano nell'elenco.
