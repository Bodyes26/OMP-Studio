...
### 9.1 Browser Studio (Gate R23 — S38-S44 completati)
...
1. **Backend Rust (`src-tauri/src/browser_live.rs`):** gestisce `browser_live_connect`/`browser_live_disconnect`/`browser_live_send_message`, verifica endpoint strettamente su loopback, riscatta il ticket monouso, decodifica i frame binari BLF1, invia ack immediati per backpressure, instrada messaggi di controllo (`takeover`, `input`, `return_control`, `set_privacy`, `inspect_point`, `set_inspector`, `request_network_body`, `clear_buffer`), applica la redazione automatica di segreti/URL credentials su console e network, e inoltra gli eventi a Svelte tramite `tauri::ipc::Channel`.
2. **Frontend Contract & Data Models (`src/lib/agent/browser-live.ts`):** parsing severo di `BrowserFrameMeta`/`BrowserTabState`, handshake capabilities `browser-live-v1`, decodifica binaria BLF1, funzioni di proiezione coordinate `mapClientToViewportCoords`/`mapWheelToViewportScroll` in pixel CSS del viewport nativo, ring buffer bounded per `ConsoleRingBuffer` (500 item con deduplicazione), `NetworkRingBuffer` (200 item con update in-place per `requestId`), `ActionRingBuffer` (100 item), parser e formattatori per l'invio selettivo del contesto al prompt (`formatElementContextForPrompt`, `formatConsoleErrorsForPrompt`, `formatFailedRequestsForPrompt`).
3. **Superficie Live & Inspector Mirato (`src/lib/components/BrowserViewer.svelte`):** rendering live ad alta fluidità del viewport, toolbar completa (navigazione, URL, permessi origini, selettore tab, preset responsive, arbitraggio controller, takeover privato, cattura screenshot, picker, inspector), overlay visivo non invasivo per l'Element Picker con tooltip semantico, dock retrattile inferiore a quattro tab (Elementi, Console, Rete, Actions) con filtri avanzati, ricerca istantanea, fetch body on-demand, gestione tastiera protetta da intercettazioni indebite e invio istantaneo del contesto al `Composer`.
